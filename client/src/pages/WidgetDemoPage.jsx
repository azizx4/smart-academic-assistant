      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto p-4">
          ...
          {user.role==="student"&&activeTab==="plan"&&plan&&(<div className="space-y-4">
            ...
            {plan.inProgress.length>0&&<div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5"><h3 className="text-sm font-semibold text-blue-700 mb-3">{t.inProgress}</h3>{plan.inProgress.map(c=>(<div key={c.courseCode} className="flex justify-between items-center p-2.5 rounded-lg mb-1.5 bg-blue-50 border border-blue-100"><div><p className="text-sm text-gray-800">{c.courseNameAr}</p><p className="text-[11px] text-gray-400">{c.courseCode}</p></div><span className="text-[11px] text-blue-600 font-medium">{c.semester}</span></div>))}</div>}
            {plan.completed.length>0&&<div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5"><h3 className="text-sm font-semibold text-emerald-700 mb-3">{t.completed}</h3>{plan.completed.map(c=>(<div key={c.courseCode} className="flex justify-between items-center p-2.5 rounded-lg mb-1.5 bg-emerald-50 border border-emerald-100"><div><p className="text-sm text-gray-800">{c.courseNameAr}</p><p className="text-[11px] text-gray-400">{c.courseCode}</p></div><span className="text-[11px] text-emerald-600 font-medium">{c.semester}</span></div>))}</div>}
            {plan.remaining.length>0&&<div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5"><h3 className="text-sm font-semibold text-gray-700 mb-3">{t.remaining}</h3>{plan.remaining.map(c=>(<div key={c.courseCode} className="flex justify-between items-center p-2.5 rounded-lg mb-1.5 bg-gray-50 border border-gray-200"><div><p className="text-sm text-gray-800">{c.courseNameAr}</p><p className="text-[11px] text-gray-400">{c.courseCode}</p></div><span className="text-[11px] text-gray-500 font-medium">{c.semester}</span></div>))}</div>}
          </div>)}

          {user.role==="student"&&activeTab==="alerts"&&(<div className="space-y-3">{alerts?.alerts?.map(a=>(<div key={a.id} className={`bg-white rounded-xl border shadow-sm p-5 ${a.type==="urgent"?"border-red-300 border-l-4":a.type==="warning"?"border-amber-300 border-l-4":"border-blue-300 border-l-4"}`}><p className="text-sm font-semibold text-gray-800">{a.title}</p><p className="text-xs text-gray-600 mt-1">{a.body}</p><p className="text-[10px] text-gray-400 mt-2">{new Date(a.createdAt).toLocaleDateString(lang==="ar"?"ar-SA":"en-US")}</p></div>))||<p className="text-sm text-gray-400 text-center py-10">{t.loadingData}</p>}</div>)}
          ...
          {user.role==="faculty"&&activeTab==="overview"&&(<div className="space-y-4">
            ...
            {facCourses&&<div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm"><h3 className="text-sm font-semibold text-gray-800 mb-3">{t.myCourses}</h3>{facCourses.map(c=>(<div key={c.code} className="flex justify-between items-center py-2.5 border-b border-gray-100 last:border-0"><div><p className="text-sm text-gray-800">{c.nameAr}</p><p className="text-[11px] text-gray-400">{c.code} {c.creditHours} {t.creditHrs}</p></div><span className="text-xs text-emerald-600 font-medium">{c.enrolledStudents} {t.enrolledStudents}</span></div>))}</div>}
            {excessiveAbs?.students?.length>0&&<div className="bg-white rounded-xl border border-red-200 p-5 shadow-sm"><h3 className="text-sm font-semibold text-red-700 mb-3">{t.studentsExceeding}</h3>{excessiveAbs.students.map((s,i)=>(<div key={i} className="flex justify-between items-center p-2.5 rounded-lg bg-red-50 border border-red-100 mb-2 last:mb-0"><div><p className="text-sm text-gray-800">{s.student.nameAr}</p><p className="text-[11px] text-gray-400">{s.course.nameAr} ({s.course.code})</p></div><span className="text-sm font-bold text-red-600">{s.absenceCount}</span></div>))}</div>}
          </div>)}

          {user.role==="faculty"&&activeTab==="courses"&&(<div className="space-y-3">{facCourses?.map(c=>(<div key={c.code+c.enrolledStudents} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm"><div className="flex justify-between items-start mb-3"><div><h3 className="text-sm font-semibold text-gray-800">{c.nameAr}</h3><p className="text-[11px] text-gray-400">{c.code} {c.creditHours} {t.creditHrs}</p></div><span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full text-[11px] font-medium">{c.enrolledStudents} {t.enrolledStudents}</span></div><div className="flex flex-wrap gap-2">{c.schedule.map((s,i)=>(<span key={i} className="px-2.5 py-1 bg-gray-100 border border-gray-200 rounded text-[11px] text-gray-600">{s.day} {s.startTime}-{s.endTime} {s.room}</span>))}</div></div>))||<p className="text-sm text-gray-400 text-center py-10">{t.loadingData}</p>}</div>)}

          {user.role==="faculty"&&activeTab==="excessiveAbs"&&(<div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden"><div className="px-5 py-3 border-b border-gray-100 bg-red-50"><h2 className="text-sm font-semibold text-red-800">{t.studentsExceeding}</h2></div>{excessiveAbs?.students?.length>0?(<div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-50 text-gray-600"><tr><th className="px-4 py-3 text-start font-medium">{t.student}</th><th className="px-4 py-3 text-start font-medium">{t.course}</th><th className="px-4 py-3 text-center font-medium">{t.absenceCount}</th></tr></thead><tbody>{excessiveAbs.students.map((s,i)=>(<tr key={i} className="border-t border-gray-100 hover:bg-red-50/30"><td className="px-4 py-3"><p className="font-medium text-gray-800">{s.student.nameAr}</p><p className="text-[11px] text-gray-400">{s.student.id}</p></td><td className="px-4 py-3"><p className="text-gray-800">{s.course.nameAr}</p><p className="text-[11px] text-gray-400">{s.course.code}</p></td><td className="px-4 py-3 text-center"><span className="px-2.5 py-1 bg-red-100 text-red-700 rounded-full font-bold">{s.absenceCount}</span></td></tr>))}</tbody></table></div>):<p className="text-sm text-gray-400 text-center py-10">{t.noData}</p>}</div>)}
